...
(function () {
  ...
  function lessonScore(l) {
    ...
    if (!U.isUnassigned(l.hwStatus)) parts.push({ w: 0.35, v: U.isHwDone(l.hwStatus) ? 1 : 0 });
    var ws = 0, vs = 0;
  ...
  function buildTeacherComment(s, courses) {
    ...
    var acc = rawAcc == null ? null : Math.max(rawAcc, accFloorFor(s)); // 展示用下限
    var name = s.name || '宝贝';
    var p = [];
  ...
  function drawShareImage() {
    ...
    if (showChips) {
      var chips = [
        { l: '综合得分', v: s.stats.score * 100, c: '#B57BFF' },
        { l: '有效听课率', v: s.stats.listen * 100, c: '#4DA3FF' },
        { l: '答题正确率', v: s.stats.accuracy != null ? Math.max(Math.round(s.stats.accuracy * 100), accFloorFor(s)) : null, c: '#3EC46D' },
        { l: '练习完成率', v: s.stats.homework != null ? s.stats.homework * 100 : null, c: '#FF7A59' }
      ];
      var n = 4, gap = 24, cw = (W - 104 - gap * (n - 1)) / n, cy = 400, ch = 128;
      chips.forEach(function (ch2, i) {
        var x = 52 + i * (cw + gap);
        ctx.fillStyle = '#fff'; rr(ctx, x, cy, cw, ch, 20); ctx.fill();
        ctx.fillStyle = ch2.c; ctx.font = '800 44px "PingFang SC",sans-serif';
        ctx.textAlign = 'left'; ctx.textBaseline = 'alphabetic';
        ctx.fillText(ch2.v == null ? '—' : Math.round(ch2.v) + '%', x + 24, cy + 64);
        ctx.fillStyle = '#6B6285'; ctx.font = '500 24px "PingFang SC",sans-serif';
        ctx.fillText(ch2.l, x + 24, cy + 100);
      });
    }
    ...
    SHARE_METRICS.forEach(function (m) {
      ...
      var pts = [];
      courses.forEach(function (cn, i) {
        var v = lessonVal(m.key, s.lessons[cn] || {});
        if (v == null) return;
        if (m.key === 'accuracy') v = Math.max(v, accFloorFor(s) / 100); // 导出展示下限
    ...
    if (!any) {
      ...
      ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
      ctx.fillText('请至少勾选一项要展示的数据', (plotL + plotR) / 2, (plotT + plotB) / 2);
    }
    ...
    ctx.fillText('学员学习数据统计工作台 · 生成于 ' + new Date().toLocaleString('zh-CN'), W / 2, fY + 52);
  }
